...
generatePath randomGen thetaSigma scaleSigma maxScale tao numSteps init =
  ...
  where
    go rGen 0 xs = (rGen, xs)
    go rGen n xs@((!x, !y, !theta, !scale):_) =
      let (deltaTheta, newGen1) = normal' (0, thetaSigma) rGen
       ...
       in if t < (1 - exp ((-1) / tao))
            then go newGen3 0 (newIndex : xs)
            else go newGen3 (n - 1) (newIndex : xs)
...
countR2S1' len numOrientations freq ts xs =
  let maxIdx = len - 1
      ...
      arr =
        ...
        L.zipWith
          (\vec t ->
             VU.map (\idx -> (idx, exp $ 0 :+ fromIntegral freq * t)) $ vec)
          ys
          ts
   in (numTrajectories, toUnboxedVector arr)
...
solveMonteCarloR2S1' numGen numTrails numPoints numOrientations freq thetaSigma tao numSteps (i, j, o, s) = do
  gens <- M.replicateM numGen newStdGen
  oris <-
    (L.map (L.map (* (2 * pi)))) <$>
...
countR2S1'' len freq1 freq ts xs =
  let maxIdx = len - 1
      ...
      numTrajectories = L.sum . L.map VU.length $ ys
      arr =
        UA.accumulate (+) 0 ((0, 0, -freq1), (maxIdx, maxIdx, freq1)) .
        VU.concat .
        L.map
          (\n ->
             VU.concat $
...
solveMonteCarloR2S1'' ::
  ...
  -> ParticleIndex
  -> IO (R.Array U DIM3 (Complex Double))
solveMonteCarloR2S1'' numGen numTrails numPoints freq1 freq thetaSigma tao numSteps (i, j, o, s) = do
  gens <- M.replicateM numGen newStdGen
  oris <-
    (L.map (L.map (* (2 * pi)))) <$>
    M.replicateM numGen (M.replicateM (div numTrails numGen) randomIO) :: IO [[Double]]
...
solveMonteCarloR2S1RP'' numGen numTrails numPoints angularFreqs angularFreqs1 radialFreqs radialFreqs1 thetaSigma scaleSigma maxScale tao numSteps (i, j, _, _) = do
  gens <- M.replicateM numGen newStdGen
  oris <-
    (L.map (L.map (* (2 * pi)))) <$>
    M.replicateM numGen (M.replicateM (div numTrails numGen) randomIO) :: IO [[Double]]
        ...
        L.map
          (fromUnboxed
             (Z :. numPoints :. numPoints :. (L.length angularFreqs) :.
              (L.length radialFreqs))) $
        ys
  return (parMap rseq (\z -> deepSeqArray z z) zs)
